    ...
    implements XMLStreamReader
{
     ...
     * {@link com.ctc.wstx.stax.cfg.InputConfigFlags},
     * (like {@link com.ctc.wstx.stax.cfg.InputConfigFlags#CONTENT_ALLOW_MIXED}).
    ...
    {
        if (mCurrToken != START_ELEMENT) {
            throwParseError("Current state not START_ELEMENT.");
        }
        String text = null;
    ...
    /**
     * Since we never return ATTRIBUTE as token type, need not
    ...
    public boolean isAttributeSpecified(int index) {
        ...
        /* Let's not bother checking index etc, since currently we can not
         * get default attribute values at all.
         */
        // !!! Needs to be fixed if/when DTDs are handled
        return true;
    }
    ...
    // // // Methods needed to construct XML events:

    public BaseNsContext constructNsContext(Location loc) {
        return mElementStack.createNonTransientNsContext(loc);
    }

    public WAttrList buildAttrList(Location loc) {
        return mAttrCollector.buildAttrList(loc);
    }
    ...
    // base implementation of initInputSource is fine as is:
    //protected void initInputSource(WstxInputSource newInput) throws IOException, WstxException

    // base implementation of loadMore() is fine as is as well
    ...
    protected void checkCData()
        throws IOException, WstxException
    {
        String wrong = checkKeyword(getNextChar(SUFFIX_IN_CDATA), "CDATA");
        if (wrong != null) {
            throwParseError("Unrecognized XML directive '"+wrong+"'; expected 'CDATA'.");
        }
        // Plus, need the bracket too:
        char c = getNextChar(SUFFIX_IN_CDATA);
        if (c != '[') {
...
*/
            ...
            if (mRootLName != null) {
                if (hasConfigFlags(CFG_SUPPORT_DTD)) {
                    if (!mElementStack.matches(mRootPrefix, mRootLName)) {
                        ...
                        throwParseError("Unexpected root element <"+str
                                        +">; expected <"
        ...
        while (i == '&') {
            ...
            if (c != CHAR_NULL) {
                ...
                if (mVldContent == CONTENT_ALLOW_NON_MIXED) {
                    ...
                    // !!! TBI: Ensure it's all white space!
                }
            ...
            if (!mCfgReplaceEntities) {
                WEntityDeclaration ed = resolveNonCharEntity
        ...
        throws IOException, XMLStreamException
    {
        ...
        boolean normalizeAttr = hasConfigFlags(CFG_NORMALIZE_ATTR_VALUES);
        ...
        while (true) {
            ...
            if (c == '/') {
                c = getNextChar(SUFFIX_IN_ELEMENT);
                if (c != '>') {
            ...
            c = (mInputPtr < mInputLen) ?
                mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_ELEMENT);
            if (c <= CHAR_SPACE) {
         ...
         */
        mVldContent = mElementStack.resolveElem(this, hasConfigFlags(CFG_INTERN_NS_URIS));
        ...
        throws IOException, XMLStreamException
    {
        ...
        mElementStack.resolveElem(this, false);
        mStEmptyElem = isEmpty;
    }
        ...
        throws IOException, XMLStreamException
    {
        ...
        char c = (mInputPtr < mInputLen) ? mInputBuffer[mInputPtr++]
            : getNextChar(SUFFIX_IN_CLOSE_ELEMENT);
        ...
        throws IOException, XMLStreamException
    {
        ...
        if (c == '-' && getNextChar(SUFFIX_IN_DOC) == '-') {
            mStTokenUnfinished = true;
            return COMMENT;
        }
        ...
        case PROCESSING_INSTRUCTION:
            {
                while (true) {
                    char c = (mInputPtr < mInputLen)
                        ? mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_PROC_INSTR);
                    if (c == '?') {
                        do {
                            c = (mInputPtr < mInputLen)
                                ? mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_PROC_INSTR);
        ...
        throws IOException, XMLStreamException
    {
        ...
        readTokenComment2(mTextBuffer);
    }

    private void readTokenComment2(TextBuffer tb)
        throws IOException, XMLStreamException
    {
        ...
        while (true) {
            char c = (mInputPtr < mInputLen) ?
                mInputBuffer[mInputPtr++] : getNextChar(SUFFIX_IN_COMMENT);
            ...
            if (c <= CHAR_CR_LF_OR_NULL) {
                ...
                c = getNextChar(SUFFIX_IN_COMMENT, tb);
                if (c == '-') { // Ok, has to be end marker then:
                    // Either get '>' or error:
                    c = getNextChar(SUFFIX_IN_COMMENT);
                    if (c != '>') {
        ...
        throws IOException, XMLStreamException
    {
        ...
        if (c <= CHAR_SPACE) {
            ...
            c = getNextChar(SUFFIX_IN_PROC_INSTR);
            if (c != '>') {
        ...
        /* Let's actually push back the char, to be included as full PI
        ...
        mTextBuffer.resetWithCopy(inputBuf, start, ptr-start);
        readTokenPI2(mTextBuffer);
    }

    private void readTokenPI2(TextBuffer tb)
        throws IOException, XMLStreamException
    {
        char[] inputBuf = mInputBuffer;
        int inputLen = mInputLen;
        ...
        while (true) {
            ...
            if (c <= CHAR_CR_LF_OR_NULL) {
                ...
                while (true) {
                    c = (mInputPtr < mInputLen) ? mInputBuffer[mInputPtr++]
                        : getNextChar(SUFFIX_IN_PROC_INSTR);
                    if (c == '>') { // got it!
                        break main_loop;
                    } else if (c == '?') {
                        if (outPtr >= outBuf.length) { // need more room?
